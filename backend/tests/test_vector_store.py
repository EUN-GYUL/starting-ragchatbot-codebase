...
class TestVectorStore:
    ...
    def test_max_results_zero_bug(self, temp_dir):
        ...
        course = Course(
            ...
            lessons=[Lesson(lesson_number=1, title="Test Lesson", lesson_link="http://example.com/1")]
        )
        chunks = [
            CourseChunk(
        ...
        assert results.is_empty(), "MAX_RESULTS=0 should return no results (reproducing bug)"
    ...
    def test_search_nonexistent_course(self, real_vector_store):
        ...
        results = real_vector_store.search("introduction", course_name="Nonexistent Course")
