...
function addMessage(content, type, sources = null, isWelcome = false) {
    ...
    if (sources && sources.length > 0) {
        // Process sources to handle both strings and objects with links
        const sourceItems = sources.map(source => {
            if (typeof source === 'object' && source.text && source.link) {
                ...
                // Handle plain text sources (backward compatibility)
                return typeof source === 'string' ? source : source.text || 'Unknown source';
            }
        });
